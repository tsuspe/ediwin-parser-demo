...
# ================== RUTAS DE SALIDA (VERSIÓN DEMO) ==================
# Todo se genera en carpetas locales dentro del proyecto.
BASE_OUTPUT_DIR   = "output"
EUROFIEL_BASE_DIR = os.path.join(BASE_OUTPUT_DIR, "eurofiel")
ECI_BASE_DIR      = os.path.join(BASE_OUTPUT_DIR, "eci")

os.makedirs(EUROFIEL_BASE_DIR, exist_ok=True)
os.makedirs(ECI_BASE_DIR, exist_ok=True)
# ====================================================================
...
def parse_order_eurofiel(order_text: str):
    ...
    # --- detectar TIPO recorriendo las líneas ---
    tipo = ""
    for ln in lines:
        up = ln.strip().upper()
        if (
            up.startswith("PEDIDO")
            or up.startswith("REEMPLAZO")
            or up.startswith("ANULACIÓN")
            or up.startswith("ANULACION")
            ):
            tipo = up
            break

        if not tipo:
            # Fallback razonable si no encontramos nada
            tipo = "PEDIDO"
...
def create_eurofiel_folders_and_pdfs(df: pd.DataFrame, pdf_bytes: bytes, base_dir: str):
    ...
    base_dir: ruta base donde crear las carpetas (en la demo: "output/eurofiel")
    """
...
def create_eci_folders_and_pdfs(df: pd.DataFrame, pdf_bytes: bytes, base_dir: str):
    ...
    Crea una carpeta por MODELO en ECI y guarda dentro:
      ...
      - Un Excel con el resumen filtrado a ese MODELO,
        con bordes, cabeceras amarillas y fila TOTAL (pedidos / unidades).
    base_dir: ruta base donde crear las carpetas (en la demo: "output/eci")
    """
    os.makedirs(base_dir, exist_ok=True)
...
def split_ediwin_txt_files_per_model(
    ...
    """
    Reparte los TXT de EDIWIN por modelo, creando/copiendo en cada carpeta de modelo:
      - LINPED_*.TXT
      - CABPED_*.TXT
      - LOCPED_*.TXT
      - OBSPED_*.TXT
    ...
    Usa LINPED para:
      - detectar modelo en cada línea (campo 7 → '3DM170/ESTAMPADO/XS')
      - mapear nº de pedido → modelo(s)
    Luego filtra el resto de ficheros por nº de pedido.
    """
    ...
    def decode_lines(b: bytes | None) -> list[str]:
        ...
        # keepends=True para respetar los saltos originales (CRLF)
    ...
    # --- 2) De LINPED sacamos: pedido -> modelos, y líneas LINPED por modelo ---
    pedido_to_models: dict[str, set[str]] = defaultdict(set)
    lin_by_model: dict[str, list[str]] = defaultdict(list)
    ...
    for line in lin_lines:
        ...
        pedido = parts[0].strip()
        modelo_field = parts[6].strip()  # '3DM170/ESTAMPADO/XS'
        if not pedido or not modelo_field:
            continue

        modelo = modelo_field.split("/")[0].strip()  # → '3DM170'
        if not modelo:
            continue

        pedido_to_models[pedido].add(modelo)
        lin_by_model[modelo].append(line)

    # Si no hay modelos, nada que hacer
    if not lin_by_model:
        return

    # --- 3) Helper para repartir líneas por modelo usando el nº de pedido ---
    def distribute_by_pedido(lines: list[str]) -> dict[str, list[str]]:
        ...
        for line in lines:
            ...
            if not pedido:
                continue
            modelos = pedido_to_models.get(pedido)
            if not modelos:
                continue
            for m in modelos:
                result[m].append(line)
        return result

    cab_by_model = distribute_by_pedido(cab_lines)
    loc_by_model = distribute_by_pedido(loc_lines)
    obs_by_model = distribute_by_pedido(obs_lines)
    obsl_by_model = distribute_by_pedido(obsl_lines)

    # --- 4) Lista de todos los modelos que aparecen en LINPED (núcleo) ---
    all_models = set(lin_by_model.keys())

    # --- 5) Para cada modelo, escribimos los 5 TXT dentro de su carpeta ---
    for modelo in sorted(all_models):
        folder_name = safe_fs_name(modelo)
        folder_path = os.path.join(base_dir, folder_name)
        os.makedirs(folder_path, exist_ok=True)

        # LINPED: siempre creamos fichero, aunque se quede vacío (si algo raro pasa)
        if linped_name:
            lin_path = os.path.join(folder_path, linped_name)
            with open(lin_path, "w", encoding=encoding, newline="") as f:
                f.writelines(lin_by_model.get(modelo, []))

        # CABPED
        if cabped_name:
            cab_path = os.path.join(folder_path, cabped_name)
            with open(cab_path, "w", encoding=encoding, newline="") as f:
                f.writelines(cab_by_model.get(modelo, []))

        # LOCPED
        if locped_name:
            loc_path = os.path.join(folder_path, locped_name)
            with open(loc_path, "w", encoding=encoding, newline="") as f:
                f.writelines(loc_by_model.get(modelo, []))

        # OBSPED
        if obsped_name:
            obs_path = os.path.join(folder_path, obsped_name)
            with open(obs_path, "w", encoding=encoding, newline="") as f:
                f.writelines(obs_by_model.get(modelo, []))

        # OBSLPED
        if obslped_name:
            obsl_path = os.path.join(folder_path, obslped_name)
            with open(obsl_path, "w", encoding=encoding, newline="") as f:
                f.writelines(obsl_by_model.get(modelo, []))

def split_ediwin_txt_files_per_model_eurofiel(
    base_dir: str,
    df: pd.DataFrame,                 # <- ya no lo usamos, pero lo dejo para no tocar la llamada
    linped_bytes: bytes,
    linped_name: str,
    cabped_bytes: bytes | None = None,
    cabped_name: str | None = None,
    ...
    - Del LINPED cogemos:
        ...
        cod_prov   = campo 6  → '3DM170/ESTAMPADO/XS' → MODELO = '3DM170/ESTAMPADO'
    - Carpeta = safe_fs_name("{MODELO}_{PATRON}") → '3DM170-ESTAMPADO_7603709-19'
    ...
    for line in lin_lines:
        ...
        cod_prov = parts[6].strip()  # 3DM170/ESTAMPADO/XS
        ...
        # MODELO: quitamos la talla → 3DM170/ESTAMPADO
        ...
        if df.empty:
            ...
            if cliente == "Eurofiel":
                ...
                if st.button("📂 Crear carpetas y PDFs por modelo en Eurofiel"):
                    try:
                        output_dir = EUROFIEL_BASE_DIR  # "output/eurofiel"
                        os.makedirs(output_dir, exist_ok=True)

                        create_eurofiel_folders_and_pdfs(
                            df, uploaded_pdf.getvalue(), output_dir
                        )
                        st.success(
                            f"Carpetas y PDFs creados en '{output_dir}'."
                        )
                ...
                if st.button("📂 Crear carpetas y PDFs por modelo en ECI"):
                    try:
                        output_dir = ECI_BASE_DIR  # "output/eci"
                        os.makedirs(output_dir, exist_ok=True)
                ...
                if st.button("📂 Repartir TXT EDIWIN por modelo en carpetas ECI"):
                    if lin_file is None:
                        ...
                        try:
                            ...
                            st.success(
                                f"TXT repartidos por modelo en '{output_dir}'."
                            )
                        except Exception as e:
                            st.error(f"Error repartiendo TXT por modelo: {e}")
